...
--
-- Host: localhost:3306
-- Generation Time: May 04, 2025 at 12:18 AM
...
CREATE TABLE `customers` (
  ...
  `payment_limit_days` int NOT NULL DEFAULT '15' COMMENT 'Số ngày giới hạn thanh toán'
...
CREATE TABLE `financial_transactions` (
  `id` int NOT NULL,
  `type` enum('income','expense') NOT NULL COMMENT 'Loại phiếu: thu hoặc chi',
  `amount` decimal(15,2) NOT NULL COMMENT 'Số tiền',
  `description` text NOT NULL COMMENT 'Mô tả phiếu',
  `status` enum('pending','approved') DEFAULT 'approved' COMMENT 'Trạng thái: chờ duyệt hoặc đã duyệt',
  `created_by` int NOT NULL COMMENT 'Người tạo phiếu',
  `approved_by` int DEFAULT NULL COMMENT 'Người duyệt phiếu (NULL nếu chưa duyệt)',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `invoices` (
  ...
  `shipping_status` enum('pending','confirmed') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'pending',
  `sub_customer_id` int DEFAULT NULL
...
CREATE TABLE `orders` (
  ...
  `invoice_id` int DEFAULT NULL,
  `created_by` int DEFAULT NULL,
  `sub_customer_id` int DEFAULT NULL COMMENT 'ID của mã phụ khách hàng'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
--
-- Table structure for table `sub_customers`
--

CREATE TABLE `sub_customers` (
  `id` int NOT NULL,
  `customer_id` int NOT NULL COMMENT 'ID của khách hàng chính trong bảng customers',
  `sub_customer_code` varchar(50) NOT NULL COMMENT 'Mã phụ của khách hàng',
  `fullname` varchar(255) NOT NULL,
  `phone` varchar(15) NOT NULL,
  `address` text NOT NULL,
  `zalo_link` varchar(255) DEFAULT NULL,
...
ALTER TABLE `invoices`
  ...
  ADD KEY `sub_customer_id` (`sub_customer_id`);
...
ALTER TABLE `orders`
  ...
  ADD KEY `idx_sub_customer_id` (`sub_customer_id`);
...
--
-- Indexes for table `sub_customers`
--
ALTER TABLE `sub_customers`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `sub_customer_code_unique` (`sub_customer_code`),
  ADD KEY `customer_id` (`customer_id`);
...
--
-- AUTO_INCREMENT for table `sub_customers`
--
ALTER TABLE `sub_customers`
  MODIFY `id` int NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `financial_transactions`
  ADD CONSTRAINT `financial_transactions_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`) ON DELETE RESTRICT,
  ADD CONSTRAINT `financial_transactions_ibfk_2` FOREIGN KEY (`approved_by`) REFERENCES `users` (`id`) ON DELETE SET NULL;
...
ALTER TABLE `invoices`
  ...
  ADD CONSTRAINT `invoices_ibfk_3` FOREIGN KEY (`sub_customer_id`) REFERENCES `sub_customers` (`id`),
...
ALTER TABLE `orders`
  ...
  ADD CONSTRAINT `orders_ibfk_sub_customer` FOREIGN KEY (`sub_customer_id`) REFERENCES `sub_customers` (`id`) ON DELETE SET NULL;
...
--
-- Constraints for table `sub_customers`
--
ALTER TABLE `sub_customers`
  ADD CONSTRAINT `sub_customers_ibfk_1` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON DELETE CASCADE;
