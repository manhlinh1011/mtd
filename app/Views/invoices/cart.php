        ...
        <?php foreach ($customerCart as $customerId => $customerData): ?>
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h4 class="text-white"><?= esc($customerData['customer_code']) ?> - <?= esc($customerData['customer_name']) ?></h4>
                </div>
                <div class="card-body pb-0" style="background-color:rgb(250, 250, 250);">

                    <?php foreach ($customerData['sub_customers'] as $subCustomerId => $subCustomerData): ?>
                        <div class="sub-customer-section" style="margin-bottom: 20px;">
                            <div class="row">
                                <div class="col-md-6">
                                    <a href="<?= base_url('invoices/create/' . $customerId . '?sub_customer_id=' . ($subCustomerId === 'no_sub' ? '' : $subCustomerId)) ?>" class="btn btn-info btn-sm">
                                        <i class="mdi mdi-plus"></i> Tạo phiếu - <?= esc($subCustomerData['sub_customer_code']) ?>
                                    </a>
                                    - <?= count($subCustomerData['orders']) ?> đơn hàng
                                </div>
                            </div>
                            <table id="datatable" class="table table-bordered dt-responsive nowrap dataTable no-footer dtr-inline collapsed" style="border-collapse: collapse; border-spacing: 0px; width: 100%;" role="grid" aria-describedby="datatable_info">
                                ...
                                <tbody>
                                    <?php foreach ($subCustomerData['orders'] as $order): ?>
                                        <tr>
                                            <td class="text-center"><?= esc($order['order_id']) ?></td>
                                            <td class="text-center">
                                                <a href="<?= base_url('orders/edit/' . $order['order_id']) ?>" target="_blank">
                                                    <?= esc($order['tracking_code']) ?>
                                                </a>
                                            </td>
