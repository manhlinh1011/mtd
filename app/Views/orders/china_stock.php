...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div>
                <div>
                    <h4 class="mb-4">#Đơn hàng quá <span class="text-danger"><?= $days ?></span> ngày chưa về kho Việt Nam</h4>
                    <!-- Form lọc đơn giản -->
                    <form action="<?= base_url('/orders/china-stock') ?>" method="GET" class="mb-3">
                        <div class="row">
                            <!-- Lọc theo số ngày chưa về kho -->
                            <div class="col-md-2">
                                <input type="number" name="days" class="form-control" placeholder="Số ngày chưa về kho" value="<?= esc($days ?? 7) ?>" min="1">
                            </div>
                    ...
                    <?php if (empty($orders)): ?>
                        <div class="alert alert-warning text-center">Không tìm thấy đơn hàng nào chưa về kho Việt Nam.</div>
                    <?php endif; ?>
                                ...
                                <table id="datatable" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        <?php foreach ($orders as $order): ?>
                                            ...
                                            // Tính số ngày chưa về từ created_at đến hiện tại (động)
                                            $createdAt = new DateTime($order['created_at']);
                                            $currentDate = new DateTime(); // Lấy ngày hiện tại từ hệ thống
                                            $daysPending = $createdAt->diff($currentDate)->days;
                                            ?>
                                            <tr>
                                                <td class="text-center"><?= $order['id'] ?></td>
                                                <td class="text-center"><?= date('Y-m-d H:i', strtotime($order['created_at'])) ?></td>
                                                <td class="text-center"><?= $daysPending ?></td> <!-- Hiển thị số ngày chưa về -->
                                                ...
                                                <td class="text-center">
                                                    <a href="<?= base_url('orders/china-stock?customer_code=' . $order['customer_code']) ?>">
                                                        <?= $order['customer_code'] ?>
                                                    </a>
                                                </td>
